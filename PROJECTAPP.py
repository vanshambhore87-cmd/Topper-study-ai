...
# Simplified connection - let the library auto-detect the best version
...
if st.button("Finding best notes"):
    if topic:
        with st.status("🔍 Searching topper database...", expanded=True) as status:
            try:
                # Switching to 2.0 Flash - The most stable 2026 model
                response = client.models.generate_content(
                    model="gemini-2.0-flash", 
                    contents=f"Explain {topic} for 10th grade {subject}. Give 3 points and 1 topper tip."
                )
                ...
                st.error(f"Try again in 30 seconds. (Error: {e})")
